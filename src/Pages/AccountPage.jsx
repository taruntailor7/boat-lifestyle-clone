...
export default function AccountPage() {
   ...
   const { loading }=useSelector((state)=>state)
    ...
    const getAdress=async()=>{
        let res1=await  fetch(`http://localhost:3001/users/${userId}/adress`)
    ...
    const handleState=async(e)=>{
       ...
       let res1= await fetch(`https://www.universal-tutorial.com/api/states/${e.target.value}`,{
        headers:{
          "Authorization":"Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyIjp7InVzZXJfZW1haWwiOiJzaGFrdGlwcmFzYWRiZWh1cmE3N0BnbWFpbC5jb20iLCJhcGlfdG9rZW4iOiJWeVZIVUZ2MmtyY0JJWGs2aXNMSGljTm1KTXB2MW90VmtGN043YWw5M0Z0cGRBMkE4cGlzaWs2Rkp3ZXFxdjBHRXNVIn0sImV4cCI6MTY2NjA2NzI1MX0.xwL7YtMbLTVsm0JrA2MOlWG7m_HY6fH9JKYzxqlDD3M",
  ...
  const handleSubmit=async(e)=>{
    ...
    if(!status){
       ...
       await  fetch(`http://localhost:3001/users/${userId}/adress`,{
            method:'POST',
  ...
  const handleDelete=async(id)=>{
    await  fetch(`http://localhost:3001/adress/${id} `,{
        method:"DELETE"
    })
    getAdress();
    toast({
            ...
            {userAdress.map((elem)=>(
              <GridItem key={elem.id} p={3} border='1px solid #c7c7c7' >
               ...
               <Text>{elem.street}</Text>
                ...
                <Text>{elem.state}</Text>
                <Text> Pin: {elem.pin} </Text>
                <Button colorScheme='red'  onClick={()=>handleDelete(elem.id)} >Delete</Button>
               </Stack>
              </GridItem>
            ))}
        
        <Box as='button' backgroundColor='#c7c7c7' ref={btnRef} onClick={onOpen}>
